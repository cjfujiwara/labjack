...
try:
    print("Loading default reference file " + defaultRefFile + " ...")
    fdata=scipy.io.loadmat(defaultRefFile)
    REFDATA=fdata['Data']
    REFTIME=fdata['Time'] 
    sr=fdata['ScanRate']
    tt=fdata['AcquisitionDate']
    
    print(" Acquisition Date  : %s" % tt[0])
    print(" Scan Rate         : %s Hz" % sr[0][0])
    print(" Data matrix shape : %s" % (REFDATA.shape,))
    print(" Time Vector Shape : %s" % (REFTIME.shape,))
...
def updateResiduePlot():    
    print("Updating residue plot objects... ")
    lbl2.config(text="Updating residue plot objects ...",bg='yellow')          

    if np.array_equal(REFTIME,PLOTTIME):
        for j in range(len(names)):
            plotsRes[j].set_data(REFTIME,REFDATA[:,j]-PLOTDATA[:,j])
    else:
        print("Unequal times. Clearing resiude plot.")
        for j in range(len(names)):
            plotsRes[j].set_data([],[])
            
    lbl2.config(text="Residue plot objects updated ...",bg='yellow') 
